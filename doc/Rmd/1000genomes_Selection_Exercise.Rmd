...
* `plink2` / `plink-1.9`: Second-generation of the multi-faceted `plink` tool.  It facilitates a number of common analyses in statistical genetics and population genetics.  Here we use it to compute $F_{ST}$ which we will need to compute the PBS statistic.  We assume you will run it using the command `plink`. 
...
plink --vcf snv_1000genomes_$tag.vcf.gz \
...
plink --vcf snv_1000genomes_${tag}_HWEfiltered.vcf.gz \
          --fst \
          --within 1kgv3_clst.txt \
...
plink --bim CHB_${tag}_HWEfiltered.bim \
