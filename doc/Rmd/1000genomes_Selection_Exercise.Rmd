...
* write pltos to files for students who can't use R interactively
* Give whole genome genetic map so they can analyze any region
* Deal with problem of normalizing iHS on region-specific files
* Polarize vcf's to derived/ancestral
...
2. Draw the 3 population tree of (CHB, FIN, YRI) and mark which branch is extended in this region. 
...
```R
# read iHS data
ihs_df <- read.table("CHB_chr2_108013601_111013601_HWEfiltered.ihs.out.10bins.norm", header = FALSE)
colnames(ihs_df) <- c("SNP", "POS", "FREQ", "ihh1", "ihh0", "unstd_iHS", "std_iHS", "top")

ggplot(ihs_df, aes(x = POS, y = FREQ)) + geom_point() + xlab('Position on chr2') + theme_bw() + ylab("|normalized iHS|") 
...
iHS_plot <- ggplot(selection_df, aes(x = POS, y = unstd_iHS)) + geom_point() + xlab('Position on chr2') + theme_bw() + ylab("|normalized iHS|") 
