...
sed '1d' integrated_call_samples_v3.20130502.ALL.panel | awk '($2 == "CHB" || $2 == "YRI" || $2 == "FIN") {print $1, $1, $2}' > chb_yri_fin_clst.txt
...
```bash
# filter out indels (-V indels), multi-allelic sites (-m 2 -M 2), keep individuals (-S) from YRI, FIN, CHB, and filter out variants with maf below 10%
# this will be used for cross-population based selection analysis
bcftools view -v snps -m 2 -M 2 -S chb_yri_fin_iids.txt -O z -q '.1 minor' 1000genomes_phase3_chr2_108013601_111013601.vcf.gz > chb_yri_fin_chr2_108013601_111013601_filtered.vcf.gz

# keep YRI individuals from the filtered vcf
# this will be used for haplotype based selection analysis.
bcftools view -S chb_iids.txt -O z chb_yri_fin_chr2_108013601_111013601_filtered.vcf.gz > chb_chr2_108013601_111013601_filtered.vcf.gz
...
fst_df <- fst_chb_yri_df %>% 
          ...
          rename(FST_CHB_YRI = FST, FST_CHB_FIN = FST.x, FST_YRI_FIN = FST.y) %>%
          select(SNP, POS, FST_CHB_YRI, FST_CHB_FIN, FST_YRI_FIN)
```

### 2.2. PBS

$$ T = -log(1 - Fst) $$
$$ PBS = \frac{T_{CHB,YRI} + T_{CHB,FIN} - T_{YRI,FIN}}{2} $$
...
needs editing still! Add some questions using the GGV
