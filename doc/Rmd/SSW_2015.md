...
# download and filter a 2MB region (1:159000000-161000000) from the 1000genomes project. Sometimes the ftp goes down if you get an error try running the command again.
tabix -h ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/20130502/ALL.chr1.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz 1:159000000-161000000 | gzip -c > 1000genomes_phase3_chr1_159000000_161000000.vcf.gz
...
bcftools view -V indels -m 2 -M 2 -S yri_ceu_chb_iids.txt -O z 1000genomes_phase3_chr1_159000000_161000000.vcf.gz > yri_ceu_chb_chr1_159000000_161000000_filtered.vcf.gz

# keep YRI individuals from the filtered vcf and remove variants below maf .1
# this will be used for haplotype based selection analysis.
bcftools view -S yri_iids.txt -q '.1 minor' -O z yri_ceu_chb_chr1_159000000_161000000_filtered.vcf.gz > yri_chr1_159000000_161000000_filtered.vcf.gz

# write bim file from vcf (-H supress header)
bcftools view yri_chr1_159000000_161000000_filtered.vcf.gz -H | awk '{print $1, $3, "0", $2, $4, $5}' > chr1_159000000_161000000_filtered.bim

# get genetic positions and interpolate genetic positions for variants not present in the genetic_map
plink --bim chr1_159000000_161000000_filtered.bim \
      --cm-map genetic_map_chr1_combined_b37.txt 1 \
      --make-just-bim \
      --out chr1_159000000_161000000_filtered_gm

# format as map file
awk '{print $1, $2, $3, $4}' chr1_159000000_161000000_filtered_gm.bim > chr1_159000000_161000000_filtered_gm.map 
```
...
plink --vcf yri_ceu_chb_chr1_159000000_161000000_filtered.vcf.gz \
      --fst \
      --within yri_ceu_clst.txt \
      --double-id \
      --id-delim . \
      --out yri_ceu_chr1_159000000_161000000_filtered \
      --set-missing-var-ids @:#
...
plink --vcf yri_ceu_chb_chr1_159000000_161000000_filtered.vcf.gz \
      ...
      --double-id \
      --id-delim . \
      --out yri_chb_chr1_159000000_161000000_filtered \
      --set-missing-var-ids @:#
...
plink --vcf yri_ceu_chb_chr1_159000000_161000000_filtered.vcf.gz \
      ...
      --double-id \
      --id-delim . \
      --out ceu_chb_chr1_159000000_161000000_filtered \
      --set-missing-var-ids @:#
```
...
fst_yri_ceu_df <- read.table('yri_ceu_chr1_159000000_161000000_filtered.fst', header = TRUE)
fst_yri_chb_df <- read.table('yri_chb_chr1_159000000_161000000_filtered.fst', header = TRUE)
fst_ceu_chb_df <- read.table('ceu_chb_chr1_159000000_161000000_filtered.fst', header = TRUE)
...
selscan --ihs \
        --vcf yri_chr1_159000000_161000000_filtered.vcf.gz \
        --map chr1_159000000_161000000_filtered_gm.map \
        --out yri_ceu_chb_chr1_159000000_161000000_filtered   
```
...
```
norm --ihs --files yri_ceu_chb_chr1_159000000_161000000_filtered.ihs.out
```
...
```r
ihs_path <- "~/jhmarcus/Teaching/SSW_2015/dev/data/yri_ceu_chb_chr1_159000000_161000000_filtered.ihs.out.100bins.norm"
...
![plot of chunk selection_scan](figure/selection_scan.png) 
